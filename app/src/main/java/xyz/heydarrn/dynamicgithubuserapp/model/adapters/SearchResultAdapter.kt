...
class SearchResultAdapter:RecyclerView.Adapter<SearchResultAdapter.SearchResultViewHolder>() {
    ...
    private val listToBind=ArrayList<ItemsItem>()

    fun observeList(list:ArrayList<ItemsItem>){
        listToBind.clear()
        listToBind.addAll(list)
...
//        }
        ...
        holder.apply {
            Glide.with(holder.itemView.context)
                .load(bindResult.avatarUrl)
                .diskCacheStrategy(DiskCacheStrategy.AUTOMATIC)
                .circleCrop()
                .into(holder.profilePicture)
            githubUsername.text=bindResult.login
            profileLink.setOnClickListener {
                val openBrowser=Intent(Intent.ACTION_VIEW, Uri.parse(bindResult.htmlUrl))
                it.context.startActivity(openBrowser)
            }
        }
    }
